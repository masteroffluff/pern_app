...
module.exports.get_wall = async function get_wall(req, res) {
    try {
        ...
        const itemModel = new ItemModel(friendsModel.at) 

        const friendsList = await friendsModel.getFriendsList(id)
        const wallResponse = await itemModel.getWall(friendsList, id)
